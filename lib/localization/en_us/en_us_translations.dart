final Map<String, String> enUs = {
  // màn hình loading Screen
  "lbl": "!",
  "lbl_c_ng_c_i_thi_n": "Cùng cải thiện",
  "lbl_gi_c_ng": "Giấc Ngủ",
  "lbl_sleep": "sleep",
  "lbl_your": "Your ",
  "msg_c_ng_c_i_thi_n_gi_c": "Cùng cải thiện Giấc Ngủ!    ",
  "msg_h_y_ng_th_t_ngon":
      "Hãy ngủ thật ngon giấc để lấy lại năng lượng, sức khoẻ cùng với một tinh thần sảng khoái!",
  "msg_let_s_get_started": "Let’s Get Started!",
  "msg_makes_you_stronger": "    makes you stronger",
  "msg_your_sleep_makes": "Your sleep    makes you stronger",

  // Input name Screen
  "lbl_age": "Age", "lbl_t_n_b_n_l_g": "Tên bạn là gì ?",

  // Suggest Screen
  "lbl_hour": "Hour",
  "msg_b_n_thu_c_tu_i":
      "Bạn thuộc độ tuổi người trưởng thành. Chúng mình đề xuất thời gian ngủ trung bình tầm 7 - 9 giờ ( Có thể tuỳ thuộc vào cơ địa, và nhu cầu của bạn )",
  "msg_h_y_nh_p_th_i_gian": "Hãy nhập thời gian ngủ trung bình 1 ngày của bạn",

  // home Screen
  "lbl_100": "100%",
  "lbl_60": "60%",
  "lbl_70": "70%",
  "lbl_80": "80%",
  "lbl_90": "90%",
  "lbl_fri": "Fri",
  "lbl_mon": "Mon",
  "lbl_sat": "Sat",
  "lbl_sun": "Sun",
  "lbl_tue": "Tue",
  "lbl_wed": "Wed",

  // homeOne Screen
  "lbl_weekly_sleep": "Weekly sleep",

  // music Screen
  "lbl_baby_sleep": "Baby Sleep",
  "lbl_bolero_90_s": "Bolero 90’s",
  "lbl_classical_sleep": "Classical Sleep",
  "lbl_for_older": "For Older  ",
  "lbl_huuthuan_ndk": "Huuthuan, NDK",
  "lbl_playlist": "Playlist",
  "lbl_xuanmike_pc": " XuanMike,PC",
  "lbl_youngh_bray": "YoungH,Bray",
  "msg_ng_s_u_gi_c_h_n": "Ngủ sâu giấc hơn",
  "msg_obito_hieuthuhai": "Obito, HieuThuHai",
  "msg_v_i_nh_ng_b_i_nh_c": "Với những bài \nnhạc dịu \ndàng!",

  // music - playlist Screen
  "lbl_deny_the_truth": "Deny The Truth ",
  "lbl_huuthuan_ndk2": "Huuthuan,NDK",
  "lbl_play": "Play",
  "lbl_sense_of_doubt": "Sense Of Doubt",
  "lbl_slither": "Slither",
  "lbl_the_bitter_end": "The Bitter End",
  "lbl_unsettling": "Unsettling",
  "msg_22_tracks_32": "22 tracks \t• 32 min",
  "msg_breathless_feat": "Breathless (Feat. Toni Coe-Brooker)",
  "msg_end_of_me_feat": "End Of Me (Feat. Brendan Murphy)",
  "msg_twisting_the_knife": "Twisting The Knife",

  // music - song Screen
  "lbl_01_12": "01:12", "lbl_share_track": "Share track",

  // music - đếm giờ để tắt nhạc Screen
  "msg_ch_nh_th_i_gian": "Chỉnh thời gian tắt nhạc",

  // tracking giấc ngủ Screen
  "lbl_07_12_am": "07:12 AM",
  "lbl_10_12_pm": "10:12 PM",
  "lbl_2h": "2h",
  "lbl_3h": "3h",
  "lbl_5h": "5h",
  "lbl_6_2": "6.2 ",
  "lbl_6_22": "6.2",
  "lbl_6h_52m": "6h 52m",
  "lbl_b_t_u_d_y": "Bắt đầu dậy",
  "lbl_b_t_u_i_ng": "Bắt đầu đi ngủ",
  "lbl_c_c_tips_cho": "Các tips cho",
  "lbl_gi_c_ng2": "giấc ngủ ",
  "lbl_gi_ng_y": "giờ/ ngày",
  "lbl_h_m_nay_th_7": "Hôm nay,  Thứ 7",
  "lbl_mn": "Mn",
  "lbl_sn": "Sn",
  "lbl_th": "Th",
  "lbl_tu": "Tu",
  "lbl_wd": "Wd",
  "lbl_z": "Z",
  "msg_24_th_ng_t_2023": "24 Tháng tư 2023",
  "msg_ch_t_l_ng_gi_c": "Chất lượng giấc ngủ",
  "msg_t_ng_s_th_i_gian": "Tổng số thời gian đã ngủ",
  "msg_th_i_gian_ng_trung": "Thời gian ngủ trung bình 1 tuần",
  "msg_th_ng_tin_c_a_gi_c": "Thông tin của giấc ngủ gần đây",

  // các tips  Screen
  "msg_c_gi_c_ng_t_t":
      "Để có giấc ngủ tốt thì cũng khá khó khăn, ngay cả khi bạn tập trung vào việc thực hiện đúng những tips tăng cường giấc ngủ đúng cách - như uống đồ uống thư giãn vào buổi tối, thực hiện các động tác duỗi cơ nhẹ nhàng và thực hiện một nghi lễ vào buổi tối, để bắt đầu. Bởi vì giấc ngủ hoạt động phù hợp với nhịp sinh học hằng ngày của cơ thể ...",
  "msg_m_i_ng_i_c_n_m_t":
      "Mỗi người cần một lượng giấc ngủ khác nhau. Tám phần một giờ là trung bình cho người lớn. Một số người có thể chịu đựng tốt với ít giấc ngủ hơn rất nhiều, trong khi một số khác cần nhiều giấc ngủ hơn mỗi đêm.",
  "msg_nh_ng_nghi_n_c_u": "Những nghiên cứu khoa học về giấc ngủ",
  "msg_nh_ng_th_ng_tin": "Những thông tin đáng chú ý",
  "msg_t_c_ng_c_a_gi_c":
      "Tác động của giấc ngủ đối với sức khỏe: Nhiều nghiên cứu đã chỉ ra rằng việc có giấc ngủ đủ giấc và chất lượng tốt liên quan mật thiết đến nhiều khía cạnh của sức khỏe, bao gồm hệ miễn dịch, tâm lý, trí não, tim mạch và cân nặng. Đồng hồ sinh học và chu kỳ giấc ngủ: Nghiên cứu về đồng hồ sinh học của cơ thể đã giúp hiểu rõ hơn về cách chu ...",

  // khi ấn vô các tips Screen
  "msg_c_gi_c_ng_t_t2":
      "Để có giấc ngủ tốt thì cũng khá khó khăn, ngay cả khi bạn tập trung vào việc thực hiện đúng những tips tăng cường giấc ngủ đúng cách - như uống đồ uống thư giãn vào buổi tối, thực hiện các động tác duỗi cơ nhẹ nhàng và thực hiện một nghi lễ vào buổi tối, để bắt đầu. Bởi vì giấc ngủ hoạt động phù hợp với nhịp sinh học hằng ngày của cơ thể (còn được gọi là đồng hồ cơ thể 24 giờ), mà có thể bị ảnh hưởng bởi các hoạt động ban ngày và ban đêm khác nhau. Sau đây là 4 thói quen xấu có thể ảnh hưởng tới giấc ngủ của bạn :\n\nSử dụng điện thoại hoặc thiết bị điện tử trước khi đi ngủ: Ánh sáng xanh từ các thiết bị này có thể làm gián đoạn chu kỳ giấc ngủ tự nhiên của cơ thể, gây khó khăn trong việc zzz.\nUống đồ uống kích thích hoặc uống cà phê vào buổi tối: Caffeine hoặc các đồ uống kích thích khác có thể gây ảnh hưởng lên việc zzz của bạn.\nThiếu chuẩn bị cho một giấc ngủ tốt: Không tạo ra môi trường ngủ thoải mái, không có ruti ngủ, hay không duy trì một thời gian ngủ cố định có thể ảnh hưởng đến chất lượng giấc ngủ của bạn.\nThực hiện công việc căng thẳng hoặc tập thể dục vào buổi tối muộn: Việc này có thể làm cho cơ thể và tâm trí bạn không thể thư giãn và chuẩn bị cho giấc ngủ tốt.\nĐể cải thiện giấc ngủ, việc loại bỏ hoặc giảm thiểu những thói quen xấu này và thay thế chúng bằng thói quen tốt hơn có thể giúp bạn có giấc ngủ sâu hơn và cảm thấy khỏe mạnh hơn vào ngày hôm sau.",

  // Sleep Mode Screen
  "lbl_04_00": "04:00", "lbl_20_30": "20 : 30", "lbl_good_night": "Good Night",
  "msg_swipe_up_to_awake": "Swipe up to awake",

  // Suggest One Screen
  "msg_h_y_nh_gi_gi_c": "Hãy đánh giá giấc ngủ của bạn",

  // Sleep Report Screen
  "lbl_06_00": "06 : 00",
  "lbl_19_11_2023": "19/11/2023",
  "lbl_22_00": "22 : 00",
  "lbl_7h30min": "7h30min",
  "lbl_802": "80",
  "lbl_awake": "Awake",
  "lbl_daydream": "Daydream",
  "lbl_deep_sleep": "Deep Sleep",
  "lbl_nigth_sleep": "Nigth Sleep",
  "lbl_one": "One",
  "lbl_sleep_duration": "Sleep duration",
  "lbl_sleep_report": "Sleep Report",
  "lbl_sleep_score": "Sleep Score",
  "lbl_sleep_time": "Sleep time",
  "lbl_wake_up_time": "Wake-up time",
  "msg_quality_report_of": "Quality report of your sleep",

  // Thông báo Screen
  "lbl_5_hours_ago": "5 hours ago",
  "lbl_dreamease": "DreamEase",
  "msg_2_more_notifcations": "2 more notifcations",
  "msg_b_n_v_a_t_c": "Bạn vừa đặt được thành tựu Educated Sleeper!",

  // profile Screen
  "msg_xo_d_li_u_c_a": "Xoá dữ liệu của tôi",

  // feedbacks Screen
  "msg_ch_ng_m_nh_v_n_ang":
      "Chúng mình vẫn đang từng ngày cố gắng hết sức để cải thiện sản phẩm, nên đừng ngần ngại hãy cho chúng mình biết chất lượng sản phẩm của chúng mình ra sao!",
  "msg_h_y_nh_gi_ch_ng": "Hãy đánh giá chúng mình",
  "msg_h_y_vi_t_nh_ng_g_p": "Hãy viết những góp ý vào đây",
  "msg_xin_ch_o_ch_ng":
      "Xin Chào! Chúng mình là đội phát triển ứng dụng DreamEase, Đây là sản phẩm đầu tiên của chúng mình nên cũng có nhiều sai sót, mong rằng các bạn hãy đánh giá sản phẩm\nđể chúng mình có thể cải thiện sớm nhất có thể !",

  // Trợ giúp y tế Screen
  "msg_t_ng_i_112":
      "Tổng đài 112  : Đây là tổng đài yêu cầu trợ giúp, tìm kiếm cứu nạn trong phạm vi cả nước. Khi các bạn cần sự trợ giúp về tìm kiếm cứu hộ cứu nạn trong các tình huống chủ yếu do thiên tai gây nên như bão lụt, lũ quét, lũ ống, sạt lở... hoặc những trường hợp trên sông,...\nTổng đài 113\nĐây là tổng đài khẩn cấp về an ninh trật tự, các vụ việc cướp giật, trộm cắp, đánh nhau, bạo hành, tai nạn giao thông.\nTổng đài 114 \nĐây là số tổng đài gọi cứu hỏa, cứu nạn trong trường hợp khẩn cấp, hỗ trợ giải cứu trong các tai nạn bất ngờ: hỏa hoạn, cháy nổ hoặc bị mắc kẹt trong những tòa nhà, thang máy, dưới hầm mỏ...\nTổng đài 115 \nĐây là số gọi cấp cứu khẩn cấp trong những trường hợp sức khỏe nghiêm trọng. Khi gọi tổng đài thì chúng ta sẽ được hỗ trợ đưa đến bệnh viện để cấp cứu nhanh chóng để bảo vệ an toàn tính mạng. \n(84 4) 3.942.5706 – 11 Tổng đài Unicef.\n0909 65 80 35  Đường dây nóng hỗ trợ sức khoẻ tâm thần. ",

  // Danh hiệu Screen
  "lbl_bedtime_story": "Bedtime Story",
  "lbl_early_riser": "Early Riser",
  "lbl_night_owl": "Night Owl",
  "lbl_night_watcher": "Night Watcher",
  "lbl_relaxation_pro": "Relaxation Pro",
  "msg_consistent_sleeper": "Consistent Sleeper",
  "msg_educated_sleeper": "Educated Sleeper",
  "msg_good_night_s_sleep": "Good Night's Sleep",
  "msg_t_c_khi_b_n":
      "Đạt được khi bạn thức dậy vào lúc 8-9 giờ sáng trong vòng 14 ngày liên tiếp",
  "msg_t_c_khi_b_n2":
      "Đạt được khi bạn đi ngủ vào khoảng 9-10 giờ tối trong suốt một tháng liên tiếp",
  "msg_t_c_khi_b_n3":
      "Đạt được khi bạn duy trì một lịch trình ngủ đều đặn trong vòng 14 ngày liên tiếp.",
  "msg_t_c_khi_b_n4":
      "Đạt được khi bạn đọc toàn bộ những tips được cung cấp bởi ứng dụng.",
  "msg_t_c_khi_b_n5":
      "Đạt được khi bạn theo dõi giấc ngủ của mình trong 30 đêm liên tiếp.",
  "msg_t_c_khi_b_n6":
      "Đạt được khi bạn nghe những bài nhạc thư giãn được cung cấp bởi ứng dụng.",
  "msg_t_c_khi_ng_i":
      "Đạt được khi người dùng nghe 1 câu chuyện trước khi đi ngủ.\n",
  "msg_t_c_sau_khi": "Đạt được sau khi bạn ngủ đủ giấc 7 đêm liên tiếp",

  // Common String
  "lbl_all": "All",
  "lbl_days": "Days",
  "lbl_discover": "Discover",
  "lbl_green_mood": "Green mood",
  "lbl_hi": "Hi!",
  "lbl_home": "Home",
  "lbl_misery_sequence": "Misery Sequence",
  "lbl_months": "Months",
  "lbl_music": "Music",
  "lbl_name": "Name",
  "lbl_profile": "Profile",
  "lbl_raining": "Raining",
  "lbl_space_noise": "Space noise",
  "lbl_stream_s_sound": "Stream's sound",
  "lbl_th_ng_b_o": "Thông báo",
  "lbl_thuan": "Thuan",
  "lbl_tr_gi_p_y_t": "Trợ giúp y tế",
  "lbl_weeks": "Weeks",
  "lbl_white_noise": "White noise",
  "msg_4_th_i_quen_x_u":
      "4 Thói Quen Xấu Mà Bác Sĩ Đang Muốn Bạn Dừng Lại Vì Giấc Ngủ Của Bạn",
  "msg_bi_u_gi_c_ng": "Biểu đồ giấc ngủ",
  "msg_brutality_will_prevail": "Brutality Will Prevail",
  "msg_chillout_and_ambient": "Chillout and Ambient",
  "msg_danh_hi_u_c_a_t_i": "Danh hiệu của tôi",
  "msg_h_y_ch_n_1_b_i_nh_c": "Hãy chọn 1 bài nhạc và đi ngủ thôi!",
  "msg_riding_in_the_horse": "Riding in the horse",
  "msg_vi_t_feedbacks_cho": "Viết feedbacks cho nhà phát triển ứng dụng",

// Network Error String
  "msg_network_err": "Network Error",
  "msg_something_went_wrong": "Something Went Wrong!",
};
